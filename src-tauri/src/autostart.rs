...
pub fn set_autostart(enabled: bool) -> Result<()> {
    ...
    if enabled {
        ...
        run.set_value(APPLICATION_NAME, &path.to_string_lossy().to_string())?;
    } else {
        run.delete_value(APPLICATION_NAME)?;
    }
...
pub fn is_autostart_enabled() -> Result<bool> {
    ...
    let value: std::result::Result<String, _> = run.get_value(APPLICATION_NAME);
    Ok(value.is_ok())
}
